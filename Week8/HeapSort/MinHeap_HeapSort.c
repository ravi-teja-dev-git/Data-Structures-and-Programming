// Heap Sort in C using Min Heap
  ...
  void heapify(int arr[], int n, int i) 
  {
    ...
    if (right < n && arr[right] < arr[smallest])//if right child is smaller than smallest
      smallest = right;
  
    // if root is not smallest,swap with smallest and continue heapify
    if (smallest != i) {
  ...
  int main() 
  {
    ...
    printf("\nImplemented sort using Min Heapify");
    heapSort(arr, n);
